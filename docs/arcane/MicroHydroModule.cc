...
extern "C" {
void _mlir_ciface_lib_main(MemRefType<uint8_t, 1> *face_coord, int64_t cid,
...
void MicroHydroModule::computeGeometricValues() {
  ...
  command << RUNCOMMAND_ENUMERATE(Cell, cid, allCells()) {
    ...
    auto memref_coord =
        make_memref_1d<uint8_t>(reinterpret_cast<uint8_t *>(coord), 24 * 8);
    ...
    _mlir_ciface_lib_main(&memref_coord, cid, &memref_out_caracteristic_length);

    info() << "Résultat calculé : "
           << m_caracteristic_length.asArray().data()[cid];
    info() << "Résultat attendu : " << expected;
    info() << "Différence       : "
           << (m_caracteristic_length.asArray().data()[cid] - expected);
    assert((m_caracteristic_length.asArray().data()[cid] - expected) < 1e-8 &&
           "Résultat différent");
