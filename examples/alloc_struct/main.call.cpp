...
extern "C" {
MemRefType<u_int8_t, 1> _mlir_ciface_lib_main();
}
struct Real3 {
  double_t x;
  double_t y;
...
int main() {
  ...
  MemRefType<u_int8_t, 1> descriptor = _mlir_ciface_lib_main();
  ...
  // TESTS
  std::cout << "x EXPECTED '" << expected.x << "', got '" << mystruct->x << "'"
            << std::endl;
  std::cout << "y EXPECTED '" << expected.y << "', got '" << mystruct->y << "'"
            << std::endl;
  std::cout << "z EXPECTED '" << expected.z << "', got '" << mystruct->z << "'"
            << std::endl;
  return 0;
}
